...
исторических MANUAL-пакетов. Коммиты `8d9bf73` и `1d3f0d6` отправлены в
`origin/dev` после подтверждения пользователя. Публикация релиза и изменение
support matrix не выполнялись. Непроверенная Windows-часть вынесена в отдельный
[Windows-чек-лист](windows-release-checklist.md).

После этой проверки временные тестовые каталоги, локальный архив
`.release-evidence/` и `Amby Manual.app` по запросу пользователя перемещены в
Корзину macOS. Их можно восстановить до очистки Корзины; исходные пути больше
не действуют. Ниже сохранён исторический результат приёмки, а актуальная очередь
незавершённых задач находится в [AUDIT_ISSUES.md](../AUDIT_ISSUES.md).

## Проверяемая сборка

- Ветка `dev`: накопленные исправления MANUAL-01—07, PERF-01 и первоначальный
  FINAL-02 зафиксированы checkpoint-коммитом `8d9bf73`. Исправление macOS bundle
  signing и regression test зафиксированы коммитом `1d3f0d6`.
...
- Это корректно ad-hoc подписанная локальная сборка, **не Developer ID signed /
  notarized distribution-ready релиз**. Архивные копии DMG перемещены в Корзину
  вместе с `.release-evidence/`; обычная production-сборка в
  `src-tauri/target/release/bundle/` при очистке не затрагивалась.
...
После явного подтверждения пользователя push коммитов `8d9bf73` и `1d3f0d6`
в `https://github.com/MrDarkOm/amby-notes.git`, ветку `dev`, выполнен успешно.
Удалённый CI для этих изменений не подтверждён.
...
Таблица фиксирует доступность файлов **на момент FINAL-02 до очистки**.
Доступный тогда локальный архив теперь находится в Корзине; записи подробного
журнала сохранены в Git (`1d3f0d6:AUDIT_ISSUES.md`).
...
3. Полная локальная ad-hoc подпись исправлена, но доверенная production
   ...
   workflow. Удалённые проверки выпускаемой ревизии также нужно подтвердить.
...
## Документы после очистки

- [AUDIT_ISSUES.md](../AUDIT_ISSUES.md): только незавершённые задачи.
- [Windows-чек-лист](windows-release-checklist.md): самостоятельный протокол
  предстоящей проверки пользователем.
- Полный прежний аудит и журнал: `git show 1d3f0d6:AUDIT_ISSUES.md`.
- Временные vault, runtime-копия, логи, screenshots, архивные DMG и
  `checkpoint.patch` перенесены в Корзину. Регрессионные тесты и постоянные
  fixtures в репозитории сохранены. Git содержит историю документов и кода,
  но не резервную копию удалённых из рабочих каталогов raw evidence.

После Windows-прогона, восстановления evidence и решения Developer ID signing
