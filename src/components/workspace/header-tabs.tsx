    ...
    <header className="flex h-10 select-none items-stretch border-b border-zinc-800 bg-[#0A0A0A]">
      ...
      {isLeftSidebarOpen && (
        <div className="flex w-52 shrink-0 items-center border-r border-zinc-800 px-3" onMouseDown={handleDragStart}>
          ...
          >
            <button
              className="flex items-center gap-1.5 rounded px-2 py-1 text-sm transition-colors hover:bg-zinc-800"
              onMouseDown={e => e.stopPropagation()}
            >
              <span className="font-medium text-zinc-200">{vaultName ?? "Workspace"}</span>
      ...
      {/* Tabs area */}
      <div className="flex flex-1 items-center justify-between overflow-hidden px-1">
          ...
          <div className="flex h-full items-center overflow-hidden">
            {tabs.map(tab => (
              <div
                ...
                onMouseDown={e => { if (e.button === 1) { e.preventDefault(); onTabClose(tab.key) } }}
                className={cn(
