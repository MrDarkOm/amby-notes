...
export function DatabasesPanel({ databaseRuntimeEnabled, onOpenDatabase }: PanelRenderProps) {
  ...
  const visibleDatabases = databases.filter((database) =>
    database.title.toLocaleLowerCase().includes(normalizedQuery),
  )

  return (
    <div className="flex min-h-0 flex-1 flex-col overflow-hidden">
      <PanelHeader
        title={t("panels.databases")}
        actions={
      ...
      />
      {databaseRuntimeEnabled && databases.length > 0 && (
        <PanelSearch
          value={query}
          onChange={setQuery}
          ariaLabel={t("databasePanel.search")}
          placeholder={t("databasePanel.search")}
        />
      )}
      {!databaseRuntimeEnabled ? (
        ...
        <div className="flex flex-1 items-center justify-center px-5 text-center">
          <p className="text-[11px] text-muted-foreground">{t("databasePanel.noResults")}</p>
        </div>
      ) : (
