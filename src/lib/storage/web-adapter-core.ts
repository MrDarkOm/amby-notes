import { StorageOperationError } from "./operation-error"
...
import {
  webHistoryPreview,
  webHistoryStats,
  webHistoryUnavailable,
  webTrashUnavailable,
} from "./web-history"
import { searchWebNotes } from "./web-search"
...
// Keep browser persistence failures at one boundary. Individual ports never
// expose browser quota/security exceptions to workspace callers.
function webGet(key: string): string | null {
  return withWebStorage(() => localStorage.getItem(key))
}

function webSet(key: string, value: string): void {
  withWebStorage(() => localStorage.setItem(key, value))
}

function webRemove(key: string): void {
  withWebStorage(() => localStorage.removeItem(key))
}
...
function webSaveTree(tree: TreeItem[]) {
  webSet(TREE_KEY, JSON.stringify(tree))
}
...
function webMoveFileContent(oldPath: string, newPath: string) {
  ...
  const content = webGet(FILE_PREFIX + oldPath) ?? webDefaultContent(oldPath)
  webSet(FILE_PREFIX + newPath, content)
  webRemove(FILE_PREFIX + oldPath)
}
...
/**
 * Browser storage port implementation. Kept behind the public adapter façade
...
export class WebAdapterCore implements StoragePort {
  ...
  async loadVaultData(_vaultPath: string): Promise<LoadVaultResult> {
    ...
    const notes = flattenWebNotes(tree).map((item) => ({
      ...
      title: item.name,
      wordCount: (webGet(FILE_PREFIX + item.id) ?? "").split(/\s+/).filter(Boolean).length,
    }))
    return {
      generation: 0,
      vaultPath: WEB_VAULT,
  ...
  async searchNotes(query: string): Promise<SearchResult[]> {
    return searchWebNotes(query, webGetTree(), (noteId) => this.readNote(WEB_VAULT, noteId))
  }
  ...
  async readFile(path: string): Promise<string> {
    ...
    throw new StorageOperationError("notFound", i18n.t("errors.pathNotFound", { path }))
  }
  ...
  async writeFile(path: string, content: string): Promise<void> {
    webSet(FILE_PREFIX + path, content)
  }
  ...
  async saveConflictCopy(path: string, content: string): Promise<string> {
    ...
    while (webGet(FILE_PREFIX + copyPath) !== null) {
      copyPath = `${stem}.${crypto.randomUUID()}-conflict${extension}`
    }
    webSet(FILE_PREFIX + copyPath, content)
    return copyPath
  }
  ...
  async createNote(
    ...
    if (webFindItem(tree, primaryPath))
      throw new StorageOperationError(
        "alreadyExists",
        i18n.t("errors.noteExists", { path: primaryPath }),
      )
    ...
    webSet(FILE_PREFIX + primaryPath, "")
    webSaveTree(nextTree)
  ...
  async createFolder(vaultPath: string, name: string): Promise<string> {
    ...
    if (webFindItem(tree, path)) {
      throw new StorageOperationError("alreadyExists", i18n.t("errors.noteExists", { path }))
    }
  ...
  async attachCanvasToNote(_vaultPath: string, canvasPath: string): Promise<FsMutationResult> {
    ...
    const content = webGet(FILE_PREFIX + canvasPath) ?? "{}\n"
    ...
    webRemove(FILE_PREFIX + canvasPath)
  ...
  async unlinkLayer(
    ...
    const content = webGet(FILE_PREFIX + oldPath)
